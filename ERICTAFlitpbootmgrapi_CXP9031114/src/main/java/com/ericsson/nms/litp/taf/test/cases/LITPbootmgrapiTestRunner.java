...
public class LITPbootmgrapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPbootmgrapiTestRunner.class);
    ...
    @TestId(id = "CXP9031114-1", title = "Upgrade ERIClitpbootmgrapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpbootmgrapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("bootmgrapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031114-2", title = "Run python test cases for ERIClitpbootmgrapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpbootmgrapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpbootmgrapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031114-3", title = "Parse xml outputs from python tests")
